...
If you choose ```ngc```, NVAIE API Key is required later.  NIM requires an NVIDIA API key defined with the NVIDIA_API_KEY environment variable. An API key can be obtained by creating an account on [build.nvidia.com](https://build.nvidia.com).
